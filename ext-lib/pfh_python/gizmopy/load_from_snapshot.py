...
def load_from_snapshot(value,ptype,sdir,snum,particle_mask=numpy.zeros(0),axis_mask=numpy.zeros(0),
        units_to_physical=True,four_char=False,snapshot_name='snapshot',snapdir_name='snapdir',
        extension='.hdf5', no_missing_print_message=False, **kwargs):
    '''
    ...
    Arguments:
      ...
      sdir: parent directory (string) of the snapshot file or immediate snapshot 
            sub-directory if it is a multi-part file (e.g. 'output' or 
            'm12i_res7100_md/output')
    ...
    Optional:
      ...
      kwargs:  don't actually do anything in this function, but accepted for compatibility
    ...
    snum=numpy.round(snum).astype(int)
    ...
    if(fname=='NULL'): 
        if no_missing_print_message==False:
...
def check_if_filename_exists(sdir,snum,snapshot_name='snapshot',snapdir_name='snapdir',extension='.hdf5',
    four_char=False, **kwargs):
    '''
    ...
    Optional:
      ...
      kwargs:  doesn't do anything in this function, but accepted for compatibility    
    '''    
