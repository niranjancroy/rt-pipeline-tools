...
def readsnap(sdir,snum,ptype,
    ...
    vel=np.copy(pos)
    ids=np.zeros([npartTotal[ptype]],dtype=np.int32)
    ...
    if (ptype==4) and (flag_sfr>0) and (flag_stellarage>0):
        ...
        mimf=np.zeros([npartTotal[ptype],13],dtype=np.float64)
    if (ptype==3):
        ...
        if(npart[ptype]>0):
            ...
            if (ptype==0):
                ...
                if(fname_ext=='.hdf5'):
                    ...
                    #if("PhotonEnergy" in input_struct[bname].keys()):
                    #    photonegy[nL:nR]=input_struct[bname+"PhotonEnergy"]
            ...
            if (ptype==4) and (flag_sfr>0):
                if(fname_ext=='.hdf5'):                
                    if("IMFFormationProperties" in input_struct[bname].keys()):
                        mimf[nL:nR,:]=input_struct[bname+"IMFFormationProperties"]
    ...
    if (ptype==0):
        ...
        'PhotonEnergy':photonegy};
    if (ptype==3):
        return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'GrainSize':grainsize}
    if (ptype==4):
        return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'z':metal,'age':stellage,'IMFFormationProperties':mimf}
    if (ptype==5) and (skip_bh==0):
