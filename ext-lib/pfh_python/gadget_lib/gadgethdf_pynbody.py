"""
-- NOTE: this version has been modified to be compatible with the (slightly modified) 
--   P. Hopkins version of GADGET-3 output files!
...
def do_properties(sim) :
    ...
    sim.properties['z'] = atr['Redshift']
    sim.properties['a'] = 1./(1.+sim.properties['z'])
    #sim.properties['time'] = units.Gyr*atr['Time_GYR']
    sim.properties['time'] = atr['Time']
    sim.properties['omegaM0'] = atr['Omega0']
    #sim.properties['omegaB0'] = atr['OmegaBaryon']
    sim.properties['omegaB0'] = 0.045
    sim.properties['omegaL0'] = atr['OmegaLambda']
    sim.properties['boxsize'] = atr['BoxSize'] * units.Unit("kpc")

    eps = np.zeros(len(sim)) + 0.1
    sim['eps'] = eps
    sim['eps'].units = units.Unit("kpc")
    sim['vel'].units = 1.0e5*units.cm/units.s
    sim['mass'].units = 1.989e43*units.g
...
def do_units(sim) :
    ...
    vel_unit = 1.0e5 * units.cm/units.s
    #dist_unit = atr['UnitLength_in_cm']*units.cm
    dist_unit = 3.086e21 * units.cm
    if cosmo :
        dist_unit/=units.h
    #mass_unit = atr['UnitMass_in_g']*units.g
    mass_unit = 1.989e43 * units.g
    if cosmo:
        mass_unit/=units.h
...
def vel(sim) :
    return sim['Velocities'] *1.0e5*units.cm/units.s
...
@GadgetHDFSnap.derived_quantity
def u(sim) :
    return sim['InternalEnergy']

@GadgetHDFSnap.derived_quantity
def wind_delaytime(sim) :
    return sim['DelayTime']

@GadgetHDFSnap.derived_quantity
def sfr(sim) :
    return sim['StarFormationRate']

@GadgetHDFSnap.derived_quantity
def ne(sim) :
    return sim['ElectronAbundance']
    
@GadgetHDFSnap.derived_quantity
def nh(sim) :
    return sim['NeutralHydrogenAbundance']

@GadgetHDFSnap.derived_quantity
def alpha(sim) :
    return sim['ArtificialViscosity']

@GadgetHDFSnap.derived_quantity
def mu(sim) :
    XH=0.76; # we track this with metal species now, could do better...
    yhelium=(1.-XH)/(4.*XH); 
    return (1.+4.*yhelium)/(1.+yhelium+sim['ne'])

"""
@GadgetHDFSnap.derived_quantity
def eps(sim) :
    return 0.050 + 0.*sim['mass']
"""
...
def temp(sim) :
    ...
    Temp= MeanWeight/BoltzMann_ergs * g_minus_1 * sim['u'] * 1.e10
    return Temp
